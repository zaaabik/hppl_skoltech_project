...
def test_pytorch_dataloader_multiple_targets():
    ...
    aggregate_target_generators = [
        AggregateTargetGenerator(
            ...
            target_name=f'sum_target_mcc_code_{idx}',
            look_back=1
    ...
    aggregate_target_generators += [
        AggregateTargetGenerator(
            ...
            aggregation_function=FuncFeatureAggregation(
                field='amount_rur', aggregate_function=mean_jit
            ),
            target_name=f'mean_target_mcc_code',
            look_back=1
        ),
        AggregateTargetGenerator(
            transaction_filter=FeatureGTETransactionFilter(
                field='amount_rur', value=0.
            ),
            aggregation_function=FuncFeatureAggregation(
                field='amount_rur', aggregate_function=max_jit
            ),
            target_name=f'max_target_mcc_code',
            look_back=1
        ),
        AggregateTargetGenerator(
            transaction_filter=EqFeatureTransactionFilter(
            ...
            aggregation_function=FuncFeatureAggregation(
                field='amount_rur', aggregate_function=min_jit
            ),
            target_name=f'min_target_mcc_code',
            look_back=1
        )
    ]

    target_mixer = TargetMixerCompose([
        DividerBaseTargetMixer(
            dividend_name='min_target_mcc_code',
            divisor_name='max_target_mcc_code',
            output_target_names='min_target_mcc_code/max_target_mcc_code'
        ),
        LinearBaseTargetMixer(
            target_names=['min_target_mcc_code', 'max_target_mcc_code'],
    ...
    data = {
        "app_id": '0',
        "APPLICATION_DATE": "2044-11-29",  # может сломаться?
        "feature_arrays": data,
    }
    ...
    cProfile.runctx("list(dataloader)", None, locals(), 'jit_pytorch_multi_target_load.pstats')
